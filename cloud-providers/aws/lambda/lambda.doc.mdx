...
```sh
bit fork learnbit.apps/aws/lambda/hello-world-lambda
```
...
```json
"myorg.some-scope/aws/lambda/hello-world-lambda": {}
```
...
AWS Lambda is set in an [app plugin](https://bit.dev/apps/app-plugin)file ending with the `lambda.*` suffix as [seen in this example](https://bit.dev/learnbit/apps/aws/lambda/hello-world/~code/hello-world.lambda.ts).
